...
Pumper (async) → ch1 → Joiner pool (std::thread) → ch2 → Drainer pool (async) → Sink
                            ↑                                    |
                        FlowKnob ← FlowMaster ← Regulator ← ch3
```
...
| **FlowMaster** | The foreman's clipboard | Receives gauge readings on ch3, feeds them to the active Regulator, writes the result to the FlowKnob. Orchestrates the feedback loop. |
...
| **FlowKnob** | The valve handle | `Arc<AtomicUsize>` shared between FlowMaster (writes) and Joiners (reads). Controls how large each payload gets before flushing. |
...
| **ch3** | Latency/error readings (`GaugeReading`) | Drainers → FlowMaster |
...
│   │       ├── workers/        # Pumper, Joiner, Drainer, FlowMaster
