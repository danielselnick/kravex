...
//!
//! 📦 PidController — a generic PID controller that regulates drum size based on a setpoint.
//!
//! 🧠 Knowledge graph:
...
mod tests {
    ...
    fn the_one_where_low_cpu_means_more_bytes() {
        ...
        let the_new_flow = the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 50 }, Duration::from_millis(3000));
    ...
    /// "The CPU is on fire. Stop barreling it. STOP." — the PID controller, calmly 🔥
    #[test]
    fn the_one_where_high_cpu_means_fewer_bytes() {
        let mut the_controller = PidController::new(75.0, 131_072.0, 67_108_864.0, 4_194_304.0);

        // 📊 First, pump output up by barreling low CPU readings — give it room to decrease
        for _ in 0..10 {
            the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 50 }, Duration::from_millis(3000));
        }
        ...
        for _ in 0..20 {
            the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 95 }, Duration::from_millis(3000));
        }
    ...
    fn the_one_where_output_respects_the_guardrails() {
        ...
        for _ in 0..100 {
            the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 10 }, Duration::from_millis(3000));
        }
        assert!(
        ...
        for _ in 0..100 {
            the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 99 }, Duration::from_millis(3000));
        }
        assert!(
    ...
    fn the_one_where_ema_smoothing_prevents_whiplash() {
        ...
        the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 100 }, Duration::from_millis(3000));
        let after_high = the_controller.ema_average;

        the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 0 }, Duration::from_millis(3000));
    ...
    fn the_one_where_zero_dt_doesnt_explode() {
        ...
        let the_result = the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 80 }, Duration::from_millis(0));
        assert!(
    ...
    fn the_one_where_pid_finds_inner_peace_at_setpoint() {
        ...
        for _ in 0..100 {
            the_controller.regulate(GaugeReading::DrainResult { drum_bytes: 0, latency_ms: 75 }, Duration::from_millis(3000));
        }
