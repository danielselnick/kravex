...
| `workers` | Pipeline stages — Pumper (async read), Refiner (sync CPU), Drainer (async write) |
...
```
Source.pump() → ch1 → Refiner(cast+join) → ch2 → Drainer(drain)
```
...
```
lib.rs → AppConfig → Foreman → Workers (Pumper, Refiner, Drainer)
lib.rs → Regulators → Manometer + Governor → FlowKnob
Foreman → Source (via Pumper), Sink (via Drainer)
Refiner → Tapper + Manifold (cast feeds, assemble drums)
