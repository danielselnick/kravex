...
- Returns raw `_search` response envelopes as `Draft` — downstream caster (PitToBulk) extracts hits
...
PIT + search_after → Draft (raw _search envelope) → PitToBulk caster → Entry → Manifold → Sink
