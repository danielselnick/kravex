...
- Returns raw `_search` response envelopes as `Barrel` — downstream tapper (PitToBulk) extracts hits
...
The `_bulk` API returns HTTP 200 even when individual documents fail (mapping errors, version conflicts, shard failures). The sink parses the response body and checks the top-level `errors` boolean. When `errors: true`, it extracts only the failed NDJSON action/doc pairs and retries them — up to 10 rounds. Each round shrinks the drum to only the rejects. Successful docs are never re-sent.

- **Per-item retry**: Failed docs extracted by index correlation (NDJSON pair `i` ↔ response item `i`), re-assembled into a smaller retry drum
...
- **Malformed body**: Non-JSON 200 responses cause an error (detects reverse-proxy HTML error barrels)
...
extract_failed_pairs() → correlate NDJSON lines to response items by index → rebuild drum from failed pairs only
