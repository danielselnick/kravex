# Summary

Core library for kravex — the data migration engine. 3-stage pipeline: Pumper (async I/O) → Refiner (sync CPU, std::thread) → Drainer (async I/O). Cow-powered zero-copy, Manifold-based drum assembly, and a clean config ownership model.

# Description

`kvx-lib` provides the foundational primitives for search migration: throttling, cutover logic, retry/recovery, and adaptive throughput. This crate is consumed by `kvx-cli` and any future integrations through the `kvx_lib` Rust crate name.

# Knowledge Graph

- **Workspace member**: `crates/kvx-lib`
- **Package**: `kvx-lib`
- **Rust crate**: `kvx_lib`
- **Dependents**: `kvx-cli`
- **Dependencies**: anyhow, async-channel, figment, memchr, reqwest, serde, serde_json, tokio, tracing, async-trait, futures, indicatif, comfy-table
- **Dev-Dependencies**: wiremock, criterion, tempfile
- **Edition**: 2024
- **Modules**: `app_config`, `backends`, `taps`, `manifolds`, `workers`, `regulators`, `foreman`, `progress`

## Pipeline Architecture (current — 3-stage: Pumper → Refiner → Drainer)
```
Source.pump() → Option<String> (raw barrel)
  → ch1 (async_channel, bounded, MPMC)
  → Refiner(s) on std::thread (recv_blocking → buffer → manifold.join(buffer, Tapper) → send_blocking)
  → ch2 (async_channel, bounded, MPMC)
  → Drainer(s) on tokio (recv → sink.drain)
  → Sink (HTTP POST, file write, memory push)
```

### Why 3 stages?
- **Pumper**: async — blocked on I/O (source reads, HTTP, file)
- **Refiner**: sync std::thread — CPU-bound (JSON parsing, tapping, manifold join, buffering)
- **Drainer**: async — blocked on I/O (sink writes, HTTP, file)

Separating CPU work onto OS threads prevents starving tokio's async I/O workers.

# Key Concepts

- **Sources return `Option<String>`**: one raw barrel per call, content uninterpreted. `None` = EOF
- **Sinks are I/O-only**: accept a fully rendered drum `String`, send it
- **Refiner buffers raw barrels** by byte size, flushes via Manifold when buffer approaches `max_drum_size_bytes`
- **Tapper** (`BarrelToDraftsTapper`): per-barrel format conversion (NdJsonToBulk, Passthrough)
- **Manifold** (`ManifoldBackend`): cast + assemble in one shot:
  - ES/File → `NdjsonManifold`: items joined with `\n`, trailing `\n`
  - InMemory → `JsonArrayManifold`: `[item,item,item]`, zero serde
- **All abstractions follow the same pattern**: trait → concrete impls → enum dispatcher → from_config resolver
- **Refiner threads**: CPU-bound work on `std::thread`, not tokio. Uses `recv_blocking()`/`send_blocking()` on async_channel
- **Drainer is thin**: just recv from ch2, send to sink. No buffering, no casting, no manifold

# Notes

- API surface follows semver; breaking changes are versioned
- Tappers and manifolds are zero-sized structs — cloning per-worker is free
- All abstractions follow: trait → concrete impl → enum dispatcher → from_config resolver
- Pipeline cascade: pumper done → ch1 closes → refiners flush+exit → ch2 closes → drainers exit
