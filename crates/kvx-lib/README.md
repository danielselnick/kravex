...
Core library for kravex — the data migration engine. 3-stage pipeline: Pumper (async I/O) → Joiner (sync CPU, std::thread) → Drainer (async I/O). Cow-powered zero-copy, Manifold-based drum assembly, and a clean config ownership model.
...
Source.pump() → Option<String> (raw barrel)
  ...
  → Joiner(s) on std::thread (recv_blocking → buffer → manifold.join(buffer, Tapper) → send_blocking)
...
- **Joiner**: sync std::thread — CPU-bound (JSON parsing, tapping, manifold join, buffering)
...
- **Joiner buffers raw barrels** by byte size, flushes via Manifold when buffer approaches `max_request_size_bytes`
